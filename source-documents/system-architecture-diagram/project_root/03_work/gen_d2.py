...
def gen_conn_inter_zone(config: dict) -> str:
    ...
    # ── L2 edge → L1 RTU 盤（RedBox 雙纖入 PRP 網）────────────
    for panel in config.get("field_control", {}).get("rtu_panels", []):
        pid = panel["id"]
        feeder = panel.get("connected_to_feeder")
        if feeder:
            ...
            if panel.get("redbox") and prp:
                # RedBox 同時連接 LAN-A 與 LAN-B edge switch
                lines.append(emit_connection(
                    f"L2.lana.SW_{feeder}_A", f"L1.{pid_lower}.REDBOX_{pid}",
                    "PRP_fiber_A", comm))
                lines.append(emit_connection(
                    f"L2.lanb.SW_{feeder}_B", f"L1.{pid_lower}.REDBOX_{pid}",
                    "PRP_fiber_B", comm))
            else:
                lines.append(emit_connection(
                    f"L2.lana.SW_{feeder}_A", f"L1.{pid_lower}.RTU_{pid}",
                    "IEC61850_MMS", comm))
