...
LEGEND_COMPONENTS = [
    ...
    ("rect",    "RTU / IED",           "#F5A623", "#fff8e1", False),
    ("rect",    "Field IED / ENG WS",  "#9B9B9B", "#f5f5f5", False),
    ("hexagon", "RedBox",              "#0C3467", "#e0f4fb", False),
...
def optimize(input_path: str, output_path: str, project_yaml: str = None):
    ...
    if _raw_lx >= 0:
        ...
        _extra_h = 0   # legend 在畫布內，不需要延伸高度
        _legend_placement = "圖內空白"
    else:
        # 找不到空白：legend 放在圖表底部下方
        lx = max(10, int((canvas_w - _legend_w) // 2))
        ly = int(canvas_h) + TITLE_HEIGHT + 20
        _extra_h = _legend_h + 40
        _legend_placement = "底部下方"
    ...
    svg = update_canvas(svg, bbox, extra_h=_extra_h)
